...
{
    float x = findMod(rotation, 5.0f);
    float z = sqrt(25.0f - x * x);
    ...
    cout << rotation;
    glm::mat4 anim = glm::rotate(glm::mat4(rotation), glm::radians(angle), axis_y);

    return Projection * View * Model * anim; // Remember, matrix multiplication is the other way around
}
